...
function OutputData = IntegrateOpenSimPlant(osimModel, controlsFuncHandle,...
    timeSpan, integratorName,SimuInfo)%, integratorOptions)
...
%         tic;
    ...
    if strcmp(integratorName,'ode5')
        [Y] = integratorFunc(plantHandle, timeSpan, InitStates)%,ODE5
    else
        [Y] = integratorFunc(plantHandle, timeSpan(1),SimuInfo.Ts,timeSpan(end), InitStates)%,
    end
    ...
    OutputData.nRows = T%size(T, 1);
    OutputData.nColumns = 55%size(T, 2) + size(Y, 2);
